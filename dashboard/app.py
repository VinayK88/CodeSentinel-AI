...
h1,h2,h3{letter-spacing:-.03em;color:var(--ink);} .hero{background:linear-gradient(135deg,#fff 0%,#f8fbff 100%);border:1px solid #e5e5ea;border-radius:28px;padding:34px 38px;margin-bottom:22px;box-shadow:0 12px 30px rgba(0,0,0,.045)}
.eyebrow{color:var(--blue);font-weight:700;font-size:.82rem;letter-spacing:.12em;text-transform:uppercase}.hero h1{font-size:3.1rem;margin:.15rem 0 .35rem}.hero p{font-size:1.15rem;color:var(--muted);max-width:850px}.pill{display:inline-block;background:#eef6ff;color:#0066cc;border-radius:999px;padding:7px 12px;margin:10px 6px 0 0;font-size:.82rem;font-weight:600}
[data-testid="stMetric"]{background:#fff;border:1px solid #e5e5ea;border-radius:22px;padding:19px 20px;box-shadow:0 8px 24px rgba(0,0,0,.04)}[data-testid="stMetricLabel"]{color:var(--muted);font-weight:600}[data-testid="stMetricValue"]{color:var(--ink);font-size:2rem;font-weight:700}
...
cols = st.columns(5)
cols[0].metric("Findings", report["finding_count"], "synthetic replay")
cols[1].metric("Critical / high", report["high_risk_count"], f"{critical} critical")
cols[2].metric("Precision", f"{eval_['precision']:.0%}", "0 false positives")
cols[3].metric("Recall", f"{eval_['recall']:.0%}", "6 / 6 families")
cols[4].metric("CWE coverage", len({x['cwe'] for x in findings}), "mapped categories")
...
with b:
    ...
    st.bar_chart({"Critical":critical,"High":high,"Medium":sum(1 for x in findings if x['severity']=='medium')}, height=250)
with c:
